...
def process_record_dataset(dataset, is_training, batch_size, shuffle_buffer,
  ...
  print(" I am here in process_record_dataset0_prefetct R")
  ...
  print(" I am here in process_record_dataset1")
  ...
  if is_training and num_gpus and examples_per_epoch:
    ...
    print("total_batches",total_batches," total_examples",total_examples,"examples_per_epoch ",examples_per_epoch)
    #dataset.take(total_batches * batch_size)

  # Parse the raw records into images and labels. Testing has shown that setting
  # num_parallel_batches > 1 produces no improvement in throughput, since
  # batch_size is almost always much greater than the number of CPU cores.
  print("I am before parse reord_fn")
...
def resnet_model_fn(features, labels, mode, model_class,
  ...
  if mode == tf.estimator.ModeKeys.TRAIN:
    ...
    update_ops = tf.get_collection(tf.GraphKeys.UPDATE_OPS) #Mahtab
    train_op = tf.group(minimize_op, update_ops)#minimize_op#
  else:
    train_op = None

  #if not tf.contrib.distribute.has_distribution_strategy(): ####### change it
