...
def resnet_model_fn(features, labels, mode, model_class,
  ...
  if not tf.contrib.distribute.has_distribution_strategy():
    ...
    accuracy_T5 = tf.metrics.mean(tf.nn.in_top_k(predictions=logits, targets=tf.squeeze(labels), k=5))
  else:
    ...
    accuracy_T5 = (tf.no_op(), tf.constant(0))

  metrics = {'accuracy': accuracy,'accuracy_T5':accuracy_T5}

  # Create a tensor named train_accuracy for logging purposes
  tf.identity(accuracy[1], name='train_accuracy')
  tf.summary.scalar('train_accuracy', accuracy[1])

  tf.identity(accuracy_T5[1],name='train_top5_accu')
  tf.summary.scalar('train_top5_accu',accuracy_T5[1])
